...
const CSVFileDocument = function (file) {
    var self = {}
    ...
    var createBlips = function (file, data) {
        try {
            ...
            contentValidator.verifyHeaders()
            var blips = _.map(data, new InputSanitizer().sanitize)
            plotRadar(FileName(file), blips, 'CSV File', [])
        } catch (exception) {
            plotErrorMessage(exception)
...
function plotFileUpload(content) {
    var fileUploadForm = content.append('div')
        .attr('class', 'input-sheet__form')

    var fileUpload = fileUploadForm.append('input')
...
function loadFile(content) {
    ...
    reader.addEventListener("load",
        function () {
            ...
            var sheet = CSVFileDocument(reader.result)
            sheet.init().build()
        }
